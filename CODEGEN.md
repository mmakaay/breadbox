...
3. Get generated code in `generated/breadbox/` (constants, macros, functions, core assembly)
4. Write assembly code using `.include "breadbox.inc"` — which pulls in everything
5. Build with ca65/ld65 (include path pointed at `generated/breadbox/`)
...
├── generated/                           ← breadbox generates everything here
│   └── breadbox/                        ← ca65 include path root (-I generated/breadbox/)
...
- Assembly templates live alongside the Python code, e.g. `breadbox/components/core/templates/`
- Jinja2 processes templates → writes output to `generated/breadbox/`

The **generated directory** (`generated/breadbox/`) is the single output root:
- ca65 is pointed at it via `-I generated/breadbox/`
- `.include "breadbox.inc"` resolves to `generated/breadbox/breadbox.inc`
- `breadbox.inc` includes `"core/boot.inc"` → resolves to `generated/breadbox/core/boot.inc`
- `breadbox.inc` includes `"hardware.inc"` → resolves to `generated/breadbox/hardware.inc`
...
└── templates/               ← assembly source templates
    ├── boot.s.j2
    ├── boot.inc.j2
    ├── vectors.s.j2
    ├── vectors.inc.j2
...
config) and written to `generated/breadbox/core/`.
...
│   ├── breadbox.inc.j2                  ← master include file
│   ├── breadbox.cfg.j2                  ← linker config
│   ├── hardware.inc.j2                  ← device constants + macros
│   └── hardware.s.j2                    ← device init + scoped functions
└── components/
    ├── core/
    │   ├── device.py
    │   ├── component.py
    │   └── templates/                   ← core assembly templates
    │       ├── boot.s.j2 / boot.inc.j2
    │       ├── vectors.s.j2 / vectors.inc.j2
    │       ├── delay.s.j2 / delay.inc.j2
    │       └── cpu_shims.s.j2 / cpu_shims.inc.j2
    ├── via_w65c22/
    │   ├── device.py
    │   ├── component.py
    │   └── templates/                   ← VIA-specific templates (if needed)
    │       └── ...
    └── ...
```

Top-level templates (`breadbox/templates/`) produce the files at the
`generated/breadbox/` root. Component templates produce files in their
respective subdirectory (e.g., `generated/breadbox/core/`).
...
Pre-processed from `breadbox/components/core/templates/`:
...
- The `generated/` directory is fully managed by breadbox
- Every generated file starts with `; Auto-generated by breadbox. Do not edit.`
- Generation clears the `generated/` directory before writing (to remove stale files)
- User files outside `generated/` are never touched
...
```bash
# Assemble core files
ca65 -I generated/breadbox/ generated/breadbox/core/boot.s
ca65 -I generated/breadbox/ generated/breadbox/core/vectors.s
ca65 -I generated/breadbox/ generated/breadbox/core/delay.s
ca65 -I generated/breadbox/ generated/breadbox/core/cpu_shims.s

# Assemble generated hardware code
ca65 -I generated/breadbox/ generated/breadbox/hardware.s

# Assemble user code
ca65 -I generated/breadbox/ project.s

# Link everything
ld65 --config generated/breadbox/breadbox.cfg \
    generated/breadbox/core/*.o \
    generated/breadbox/hardware.o \
    project.o \
    -o rom.bin
```

Future: breadbox could drive the full build (ca65 + ld65) as part of a
`breadbox build` command.
...
- All generated code goes in `generated/breadbox/` subdirectory of the project
- ca65 include path: `generated/breadbox/`
- Assembly source templates live alongside their component's Python code
