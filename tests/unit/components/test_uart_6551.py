...
from breadbox.components.uart_6551 import Uart6551Device
from breadbox.components.uart_6551 import resolve
...
class TestUart6551Device:
    def test_valid_generic(self):
        device = Uart6551Device(id=ComponentIdentifier("UART0"), address=Address16("$5000"))
    ...
    def test_valid_types(self, uart_type):
        device = Uart6551Device(id=ComponentIdentifier("UART0"), type=uart_type, address=Address16("$5000"))
        assert device.type == uart_type

    def test_type_lowercased(self):
        device = Uart6551Device(id=ComponentIdentifier("UART0"), type="W65C51N", address=Address16("$5000"))
        assert device.type == "w65c51n"

    def test_invalid_type(self):
        with pytest.raises(ValueError, match="Invalid UART type"):
            Uart6551Device(id=ComponentIdentifier("UART0"), type="z80uart", address=Address16("$5000"))
    ...
    def test_irq_off(self):
        device = Uart6551Device(id=ComponentIdentifier("UART0"), address=Address16("$5000"), irq=OnOff("off"))
        assert device.irq == "off"

    def test_address_auto_coerced(self):
        device = Uart6551Device(id=ComponentIdentifier("UART0"), address="$5000")  # type: ignore
