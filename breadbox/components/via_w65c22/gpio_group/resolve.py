...
def resolve(
    ...
    if has_pins and not has_port_bits:
        # pins config: derive port + bits from pin names via the provider device.
        # Pin ordering is preserved as given by the caller — consumers may
        # assign semantic meaning to positions (e.g. LCD: [RS, RWB]).
        port, bitmask = provider_device.resolve_pins(device_settings["pins"])
        bits = Bits(bitmask)
        ...
        pins_for_port = provider_device.get_port(device_settings["port"])
    ...
    return ViaW65c22GpioGroupDevice(id=component_id, provider_device=provider_device, **device_settings)
