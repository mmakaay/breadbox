...
def resolve(config: BreadboxConfig, device_id: DeviceIdentifier, device_settings: dict[str, Any]) -> AbstractGpioPinDevice:
    ...
    try:
        ...
        raise ValueError(f"Bus type {bus_type!r} does not support gpio_pin (no module {module_name})") from None

    return module.resolve(config, device_id, bus_device, device_settings)
