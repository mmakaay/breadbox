...
; Data bus: {{ data.component_path }} (port {{ data.port }} on {{ data.bus_device.id }}, mask {{ data.bits | hex }})
;
...
{% endif %}
    ...
    ;
    ; In (zero page):
    ;   {{ symbol("byte") }} = command byte to write
    ; Out:
    ;   A = clobbered
